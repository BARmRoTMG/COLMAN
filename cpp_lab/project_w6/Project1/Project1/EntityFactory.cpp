...
Entity* EntityFactory::Create( const std::string& type, sf::RenderWindow& wnd )
{
    ...
    else if( type == "enemy_idle" )
    {
        e = new Entity( "enemy_idle" );
        ...
        e->addComponent<AIComponent>( AIComponent::BehaviorType::Idle, std::make_unique<Idle>() );
    }
    else if( type == "enemy_random" )
    {
        ...
        e->addComponent<VelocityComponent>( 0.f, 0.f, 40.f );
        e->addComponent<DrawableComponent>( m_rm, "assets/enemy.png" );
        e->addComponent<AIComponent>( AIComponent::BehaviorType::Random, std::make_unique<RandomMove>() );
    }
    else if( type == "enemy_chase" )
    {
        ...
        e->addComponent<VelocityComponent>( 0.f, 0.f, 40.f );
        e->addComponent<DrawableComponent>( m_rm, "assets/enemy.png" );
        e->addComponent<AIComponent>( AIComponent::BehaviorType::Chase, std::make_unique<ChasePlayer>( "player" ) );
    }
    else if( type == "enemy_leftright" )
    {
        e = new Entity( "enemy_leftright" );
        e->addComponent<PositionComponent>( 400.f, 100.f );
        e->addComponent<VelocityComponent>( 40.f, 0.f, 40.f );
        e->addComponent<DrawableComponent>( m_rm, "assets/enemy.png" );
        e->addComponent<AIComponent>( AIComponent::BehaviorType::LeftRight, std::make_unique<LeftRight>() );
    }
    else if( type == "text_start" )
    {
        //init the entity
        e = new Entity( "text_start" );
        //e->addComponent<PauseTagComponent>();
        e->addComponent<TextComponent>( m_rm, "assets/arial.ttf" );

        //init the txt object
        auto txt = e->getComponent<TextComponent>();
        txt->m_text.setString( "Press Enter To Start!" );
