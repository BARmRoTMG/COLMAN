...
{
    // Check for pause at the start of update
    if (m_inputSystem.isKeyPressed(sf::Keyboard::Space) ||
        m_inputSystem.isKeyPressed(sf::Keyboard::Escape))
    {
        if (m_pause)
        {
            // Clear these keys so they don't get detected when resuming
            m_inputSystem.clearAllKeys();
